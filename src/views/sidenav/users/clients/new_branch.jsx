import { Modal, Form, Input, Button, App, Row, Col, Select, Tooltip, InputNumber } from "antd";
import { useState, useEffect } from "react";
import Flex from "../../../../components/shared-components/Flex";
import { useDispatch } from "react-redux";
import { addBranch, addDocumentImage, getAdminsForClient } from "../../../../store/slices/UsersSlice";
...
const NewBranch = (props) => {
    ...
    useEffect(() => {
        dispatch(getAdminsForClient()).then((response) => {
            if (!response.error) {
                setAdmins(response.payload.map((elm) => ({
                    label: (
                        <div className="p-1">
                            <AvatarStatus
                                id={elm.id}
            ...
            dispatch(addBranch(body)).then(async (response) => {
                if (!response.error) {
                    ...
                    for (const file of image) {
                        ...
                        await dispatch(addDocumentImage({ formData, id: response.payload.newUser.id })).then((response) => {
                            if (!response.error) {
                                document_images = response.payload.document_images
                            }
                        })
                    }
    ...
    return (
        <Modal maskClosable={false} open={visible} width={900} onCancel={close} footer={null} title={`${getTranslation("sidenav.client.new_branch")}`}>
            ...
            >
                ...
                <Flex gap={10} justifyContent="flex-end">
                    ...
                    <Button type="primary" loading={isLoading} htmlType="submit">{getTranslation("sidenav.client.Add")}</Button>
                </Flex>
            </Form>
