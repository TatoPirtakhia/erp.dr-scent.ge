...
const Product = () => {
    ...
    useEffect(() => {
        ...
        dispatch(getFilteredProduct({ filterData }))
            .then((response) => {
                if (!response.error) {
    ...
    useEffect(() => {
        dispatch(getProductCategory())
            .then((response) => {
                if (!response.error) {
                    setCategoryOptions(response.payload.map((item) => ({
                        value: item.id,
                        data: item,
        ...
        dispatch(get_unit())
            .then((response) => {
                if (!response.error) {
                    setUnitOptions(response.payload.data.map(item => ({
    ...
    const tableColumns = [

        {
            ...
            render: (_, record) => (
                <div className="flex items-center gap-3 min-w-[250px]">
                    {!record.image ? <NoImage height={35} width={40} /> : <img alt='Product Image' height={45} width={45} style={{ objectFit: "contain", }} src={`${API_BASE_URL}images/products/${record.image}`} />}
        ...
        },
        {
            ...
            render: (_, record) => (
                <div className="text-center max-w-[300px]">
                    <span>{isMobile ? truncateText(record.description, 20) : record.description || '-'}</span>
                </div>
            ),
        },

        {
            ...
            render: (_, record) => (
                <Flex justifyContent='center' >
                    <span >{parseInt(record.unit_id) === -1 ? getTranslation('sidenav.product.item') : `${locale === "ka" ? record.name_ka : record.name_en}`}</span>
                </Flex>
            ),
        },
        {
            ...
            render: (_, record) => (
                <Flex justifyContent='center' >
                    <span >
                        <CountUp isCounting duration={0.7} end={record.quantity ? parseInt(record.quantity) : 0} thousandsSeparator="," />
                    </span>
                </Flex>
            ),
        },
